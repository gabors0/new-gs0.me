<script lang="ts">
  ...
  let didHoverRevealEmail = false;

  function previewEmail() {
    if (isEmailVisible) return;

    isEmailVisible = true;
    didHoverRevealEmail = true;
  }

  function clearEmailPreview() {
    if (!didHoverRevealEmail) return;

    isEmailVisible = false;
    didHoverRevealEmail = false;
  }

  function toggleEmail() {
    if (didHoverRevealEmail) {
      didHoverRevealEmail = false;
      return;
    }
...
<div class="flex flex-col justify-center items-center h-screen fade-main">
  ...
  >
    ...
    <div class="flex flex-row justify-between items-center mb-0.5">
        <h1
          class="font-suse-mono font-light text-3xl transition-[letter-spacing] {isEmailVisible
            ? ''
            : 'hover:tracking-wider'}"
        >
          <a
            href={isEmailVisible ? 'mailto:gabors0@proton.me' : undefined}
            tabindex={isEmailVisible ? undefined : -1}
            class="group"
          >
            <span class="relative inline-block">
              <span class="bg-linear-to-b from-white/90 to-white/60 bg-clip-text text-transparent">
                gabors0
              </span>
              <span
                aria-hidden="true"
                class="absolute inset-0 text-white opacity-0 transition-opacity duration-150 group-hover:opacity-100"
              >
                gabors0
              </span>
            </span>{#if isEmailVisible}<span
                class="inline-block"
        ...
        <div class="hidden sm:flex flex-row flex-wrap justify-end gap-x-1">
        <a
        ...
        <a href="https://ko-fi.com/gabors0" aria-label="ko-fi" target="_blank">
          ...
          <button
            ...
            onmouseenter={previewEmail}
            onmouseleave={clearEmailPreview}
            onclick={toggleEmail}
            ><svg
              class="svgIcon transition-all duration-75 opacity-70 hover:-translate-y-1.5 hover:opacity-100"
              xmlns="http://www.w3.org/2000/svg"
    ...
    <div class="flex flex-row opacity-50 justify-between items-center">
      ...
      <div class="flex flex-row items-baseline gap-1">
        <span class="flex flex-row items-baseline mb-0.5">
  ...
  <div
    use:spotlight
    class="spotlight relative sm:hidden flex flex-row items-center justify-center gap-x-1 w-[90%] bg-black/30 border border-t-0 border-white/30 bg-blend-darken mx-5 p-5 sm:mx-0 sm:w-xl main {areStatsVisible
      ? 'hidden-short'
      : ''}"
  >
      <a
